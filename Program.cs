...
do
{
    ...
    switch (menuSelection)
    {
        ...
        case "2":
            ...
            List<string> animalsInput = new List<string>();
            // List<string> animalsContains = new();
            ...
            while (dogCharacteristic == "")
            {
                ...
                {
                    string[] c = readResult.Split(",");
                    foreach (var i in c)
                    {
                        // Console.WriteLine(i.ToLower().Trim());
                        var x = i.ToLower().Trim();
                        animalsInput.Add(x);
                    }
                    dogCharacteristic = readResult.ToLower().Trim();
                    foreach(var i in animalsInput){
                        Console.WriteLine(i);
                    }
                    Console.WriteLine();
                }
            ...
            for (int i = 0; i < maxPets; i++)
            {
                ...
                if (ourAnimals[i, 1].Contains("dog"))
                {
                    ...
                    foreach(var f in animalsInput){
                    if (dogDescription.Contains(f))
                    {
                        // #3b update message to reflect term 
                        // #3c set a flag "this dog" is a match
                        // animalsContains.Add(f);
                        Console.WriteLine($"\nOur dog {ourAnimals[i, 3]} is a match! it contains {f}");
